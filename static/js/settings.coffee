$().ready ->
  ...
  $("#start-upgrade-btn").click (e) ->
    ...
    .done  (data, e) ->
      getStatus = (id) ->
        ...
        .done  (data, e, jqXHR) ->
          ...
          if jqXHR.status == 202
            ...
            ($ "#upgrade_logs").append "\nMediaz-Electronics Signage-Player update was finished"
      ...
      ($ "#upgrade_logs").text "Mediaz-Electronics Signage-Player upgrade has started successfully."
  ...
  $("#btn-reboot-system").click (e) ->
    if confirm "Are you sure you want to reboot your Mediaz-Electronics Signage-Player?"
      $.post "/api/v1/reboot_screenly"
      .done  (e) ->
        ($ "#request-error .alert").show()
        ($ "#request-error .alert").addClass "alert-success"
        ($ "#request-error .alert").removeClass "alert-danger"
        ($ "#request-error .msg").text "Mediaz-Electronics Signage-Player reboot has started successfully."
      .fail (data, e) ->
        ($ "#request-error .alert").show()
        ($ "#request-error .alert").addClass "alert-danger"
        ($ "#request-error .alert").removeClass "alert-success"
  ...
  $("#btn-shutdown-system").click (e) ->
    if confirm "Are you sure you want to shutdown your Mediaz-Electronics Signage-Player?"
      $.post "/api/v1/shutdown_screenly"
      .done  (e) ->
        ($ "#request-error .alert").show()
        ($ "#request-error .alert").addClass "alert-success"
        ($ "#request-error .alert").removeClass "alert-danger"
        ($ "#request-error .msg").text "Mediaz-Electronics Signage-Player shutdown has started successfully. Soon you will be able to unplug the power from your Raspberry Pi."
      .fail (data, e) ->
        ($ "#request-error .alert").show()
        ($ "#request-error .alert").addClass "alert-danger"
        ($ "#request-error .alert").removeClass "alert-success"
